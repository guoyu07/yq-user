...
elseif rs("gdgc")=0 or rs("gdgc")<0 or (zhgc+0)>rs("gdgc") then
   response.write "<script language='javascript'>"
   response.write "alert('Faulty operation, lack of bonus points, please check the input is correct!');"
   response.write "history.go(-1);"
   response.write "</script>" 
   response.end
else
...
rs_d1("sygc")="-"&zhgc
rs_d1("ljgc")=rs("gdgc")
rs_d1("bz")="To consumption options"
rs_d1.update
%>
<%
...
%>
<%
response.Write "<script language=javascript>alert('Change"&zhgc&"Success!');window.location.replace('sfjl.asp');</script>"
%>