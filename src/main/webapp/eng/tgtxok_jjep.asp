...
if rs.eof and rs.bof then
   ...
   response.write "</script>" 
   response.end
elseif tgpay*1>rs("pay")-rs("vippay")-rs("fhpay") then
   response.write "<script language='javascript'>"
   response.write "alert('Hello, you sell a currency shortage bonus, please try again, need to sell the game as part of the income of a currency gain points Please games next to a currency sell, thank you!');"
   response.write "history.go(-1);"
   response.write "</script>" 
   response.end
...
elseif rs("payok")=1 then
   ...
   response.write "<script language='javascript'>"
   response.write "alert('EP publication number can not be empty, please check the input is correct!');"
   response.write "history.go(-1);"
   response.write "</script>" 
   response.end
end if
...
%>
<%
response.Write "<script language=javascript>alert('Your publication has been successfully submitted, after the release of the success of our EP-direction of your subscription orders within two hours after the hit on your payment account! Please check your timely£¡');location.replace('epmcjl.asp');</script>"
%>