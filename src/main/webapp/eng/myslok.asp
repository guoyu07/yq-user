...
elseif rs("jydb")=0 or rs("jydb")<0 or (jo+0)>rs("jydb") then
   response.write "<script language='javascript'>"
   response.write "alert('Faulty operation, lack of coins, please check the input is correct!');"
...
rs_d1("bz")="Buy listed in"
rs_d1("jypay")=int(sl*request("gj")*1+0.1)/1
rs_d1("jyid")=rs0("id")
rs_d1.update
%>
<% 
response.Write "<script language=javascript>alert('Successful bid listing!');location.replace('gpjy.asp');</script>"
%>