...
if rs_you.eof and rs_you.bof then
   response.write "<script language='javascript'>"
   response.write "alert('Only search their own teams and players have entered the two-zone!');"
   response.write "history.go(-1);"
   response.write "</script>"
   response.end
end if
...
rel=stylesheet type=text/css href="main/style.css">
...
sql="select * from txpay where payonoff = 'ÉÐÎ´×ªÕË' and ep=0 and payusername='"&request("user")&"'" 
rs.open sql,conn1,1,1
  if rs.eof and rs.bof then 
      ...
      if (currentpage-1)*MaxPerPage>totalput then 
       ...
       if currentPage=1 then 
                       showContent 
            showpage totalput,MaxPerPage,"epjysc_user.asp" 
       else 
          if (currentPage-1)*MaxPerPage<totalPut then 
