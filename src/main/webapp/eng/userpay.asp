...
elseif rs("vip")=1 then
   ...
   response.write "</script>" 
   response.end
elseif rs("txlb")=1 or rs("txlb")=2 then
   response.redirect"userpay_new.asp"
...
<body bgcolor="#FFF3D0" text="#000000" leftmargin="0" topmargin="3" marginheight="0"><div align="center"><b>
	...
	<table border="0" width="75%" id="table1" height="58" bgcolor="#FFEEEE">
	 ...
	 <tr>
	 <td>You currently can sell a currency as follows:<b><font color="#008000"><%if rs("pay")<1 then%>0<%=rs("pay")-rs("vippay")%><%else%><%=rs("pay")-rs("vippay")%><%end if%></font></b> A currency</td>
	 </tr>
	 </table>
	 <table width="75%" border="0" cellpadding="0" class="border" bgcolor="#800080">
    <tr>
    <td align="center" valign="top" bgcolor="#FFFFFF" width="935"><form method="POST" align="center" name="Form" onSubmit="return checkdate()" action="tgtxok.asp">
    ...
    <TR class=content> 
      ...
      <TD width="571" bgColor=#ffffff height="29"><font style="font-size: 9pt" color="#FF0000">
		...
		</select>**</font><font style="font-size: 9pt">The current can sell a currency as follows:</font><font style="font-size: 9pt; " color="#FF0000"><%if rs("pay")<1 then%>0<%=rs("pay")-rs("vippay")%><%else%><%=rs("pay")-rs("vippay")%><%end if%></font></TD>
    </TR>
    <TR class=content> 
