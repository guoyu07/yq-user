...
%>
<% 
sql_jt="select sum(sjb) as jt from sgxt where DateDiff('d',date(),bddate)=0 and new>0 and sjb>9" 
Set rs_jt= Server.CreateObject("ADODB.Recordset") 
rs_jt.open sql_jt,conn7,1,1
%>
<% 
sql_zt="select sum(sjb) as zt from sgxt where DateDiff('d',date(),bddate)=-1 and new>0 and sjb>9" 
...
rs.open sql,conn7,1,1
  if rs.eof and rs.bof then 
      ...
      if (currentpage-1)*MaxPerPage>totalput then 
       ...
       if currentPage=1 then 
                       showContent 
            showpage totalput,MaxPerPage,"sqday2013.asp" 
       else 
          if (currentPage-1)*MaxPerPage<totalPut then 
