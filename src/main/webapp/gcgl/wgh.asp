...
<p align="center"><b><span style="font-size: 11pt">员工提现申请清单-工商银行</span></b></TD></TR>
  <TR> 
...
sql1="select * from txpay where payonoff = '尚未转账' and paybank='工商银行' order by payid" 
...
%>
        <tr> 
          ...
          <td height="38" width="91" align="center"><%if rs1("ep")=1 and rs1("payonoff")="尚未转账" then%><b><a onClick="return confirm('提示：您确定付款了吗？ ')" href="qxjy.asp?qx=<%=rs1("payid")%>&tx=wgh"><font color="#008000">撒消交易</font></a></b><%else%><a href="xtql.asp?payep=<%=rs1("payid")%>&tx=wgh">系统处理</a><%end if%></td>
        </tr>
<!--循环体结束
分页部分：-->
   <%  
...
<br>
   ...
   <tr>
     ...
     <%else%>
     <%end if%>
    <td width="50%" align="right" valign="middle"> <a href="wgh.asp?page=1">首页</a> | 
     <%if pre then%>
     <a href="wgh.asp?page=<%=intpage -1%>">上页</a> | <%end if%>
     <%if last then%>
      <a href="wgh.asp?page=<%=intpage +1%>">下页</a> |<%end if%>&nbsp;<a href="wgh.asp?page=<%=rs1.PageCount%>">尾页</a> |&nbsp; 转到第<select name="sel_page" onchange="javascript:location=this.options[this.selectedIndex].value;">
      <%
       for i = 1 to rs1.PageCount
       if i = intpage then%>
       <option value="wgh.asp?page=<%=i%>" selected><%=i%></option>
     <%else%>
       <option value="wgh.asp?page=<%=i%>"><%=i%></option>
        <%
          end if
        next
        %>
