...
sql1="select * from txpay where payonoff = '尚未转账' and txlb<>1 order by payid" 
...
%>
        <tr> 
          ...
          <td height="38" width="114" align="center"><%if rs1("dfuser")<>"" then%><font color="#0000FF"><b><%=rs1("dfuser")%>-</b><a onClick="return confirm('提示：您确定了吗？ ')" href="qxjyth.asp?qx=<%=rs1("payid")%>&tx=w0322"><font color="#FF0000">有误</font></a></font><%end if%></td>
          <td height="38" width="137" align="center"><%=rs1("rgdate")%></td>
          <td height="38" width="195" align="center"><%if rs1("ep")=1 then%>等待认购方付款<%elseif rs1("ep")=2 then%>等待求现方确认<%else%><font color="#FF0000">等待认购中-<a onClick="return confirm('提示：您确定了吗？ ')" href="qxepmc.asp?qx=<%=rs1("payid")%>&tx=w0322">撤销<%end if%></a></font></td>
          <td height="38" width="91" align="center"><%if rs1("ep")=1 and rs1("payonoff")="尚未转账" then%><b><a onClick="return confirm('提示：您确定付款了吗？ ')" href="qxjy.asp?qx=<%=rs1("payid")%>&tx=w0322"><font color="#008000">撒消交易</font></a></b><%else%><a href="xtql.asp?payep=<%=rs1("payid")%>&tx=w0322">系统处理</a><%end if%></td>
        </tr>
<!--循环体结束
分页部分：-->
   <%  
...
<br>
   ...
   <tr>
     ...
     <%else%>
     <%end if%>
    <td width="50%" align="right" valign="middle"> <a href="w0322.asp?page=1">首页</a> | 
     <%if pre then%>
     <a href="w0322.asp?page=<%=intpage -1%>">上页</a> | <%end if%>
     <%if last then%>
      <a href="w0322.asp?page=<%=intpage +1%>">下页</a> |<%end if%>&nbsp;<a href="w0322.asp?page=<%=rs1.PageCount%>">尾页</a> |&nbsp; 转到第<select name="sel_page" onchange="javascript:location=this.options[this.selectedIndex].value;">
      <%
       for i = 1 to rs1.PageCount
       if i = intpage then%>
       <option value="w0322.asp?page=<%=i%>" selected><%=i%></option>
     <%else%>
       <option value="w0322.asp?page=<%=i%>"><%=i%></option>
        <%
          end if
        next
        %>
