...
<table width="100%" border="1" cellpadding="2" cellspacing="0" bordercolor="#FFFFFF" style="border-collapse: collapse" height="64">
        <tr>
          ...
          <td width="128" bgColor="#C3DAF9" height="24" align="center"><b><font face="幼圆" color="#008000">实返</font></b></td>
          ...
          <td width="76" bgColor="#C3DAF9" height="24" align="center"><div class="word2" align="center">操作</div></td>
        </tr>
        <%
dim sql1 
dim rs1
dim i,intPage,page,pre,last,filepath
sql1="select * from txpay where payonoff = '尚未转账' and txvip=2 order by payid"
'sql1="select * from txpay where payonoff = '尚未转账' and txvip=2 and paytime>now()+9 order by payid" 
...
%>
        <tr> 
          ...
          <td height="38" width="80" align="center" bgcolor="#AADAA3"><b><%=rs2("name")%></b></td>
          <td height="38" width="66" align="center"><font color="#FF0000"><b><%=rs1("paynum9")%></b></font></td>
          <td height="38" width="77" align="center"><%=rs1("payname")%></td>
          <td height="38" width="128" align="center"><b><font color="#008000" face="幼圆"><%=rsf("mqfh")%>-<%=rsf("zfh")%></font></b></td>
          <td height="38" width="75" align="center" bgcolor="#DFDFDF"><b><font color="#FF0000" size="2"><%=rs_ztx("ljztx")%></font></b></td>
          <td height="38" width="158" align="center"><%=rs1("paytime")%></td>
          <td height="38" width="170" align="center"><%=rs1("paybank")%><%if rs1("ep")=1 then%><a href="syusers.asp?payep=<%=rs1("payid")%>&tx=w10">-未审核</a><%end if%></td>
          <td height="38" width="182" align="center"><%=rs1("paycard")%></td>
          <td height="38" width="76" align="center"><%if rs1("ep")=0 and rs1("payonoff")="尚未转账" then%><a href="xtql.asp?payep=<%=rs1("payid")%>&tx=w10">系统处理</a><%end if%></td>
        </tr>
<!--循环体结束
分页部分：-->
   <%  
...
<br>
   ...
   <tr>
     ...
     <%else%>
     <%end if%>
    <td width="50%" align="right" valign="middle"> <a href="w10.asp?page=1">首页</a> | 
     <%if pre then%>
     <a href="w10.asp?page=<%=intpage -1%>">上页</a> | <%end if%>
     <%if last then%>
      <a href="w10.asp?page=<%=intpage +1%>">下页</a> |<%end if%>&nbsp;<a href="w10.asp?page=<%=rs1.PageCount%>">尾页</a> |&nbsp; 转到第<select name="sel_page" onchange="javascript:location=this.options[this.selectedIndex].value;">
      <%
       for i = 1 to rs1.PageCount
       if i = intpage then%>
       <option value="w10.asp?page=<%=i%>" selected><%=i%></option>
     <%else%>
       <option value="w10.asp?page=<%=i%>"><%=i%></option>
        <%
          end if
        next
        %>
