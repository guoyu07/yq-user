...
'if request.cookies("lkuserid")="" then
   ...
   'response.write "window.location.replace('/vip/jjsydate.asp');"
...
if rs("lksl")=0 or rs("lksl")<0 then
   response.Write "<script language=javascript>alert('电子卡发放及业绩更新成功！');location.href='/vip/jjsydate.asp'</script>"
   response.end
end if
%>
<%
Function getCode(iCount)
     Dim arrChar
     Dim j,k,strCode
     arrChar = "012349abcdefg5678hjkxyzi"
     k=Len(arrChar)
     Randomize
     For i=1 to iCount
          j=Int(k * Rnd )+1
