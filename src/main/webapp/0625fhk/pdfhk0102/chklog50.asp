...
sql="select * from jbk500 where pdid = '" & pdid & "'"
rs_login.open sql,conn0,1,1
if rs_login.eof and rs_login.bof then
   response.write "<script language='javascript'>"
   response.write "alert('你的 消费编号 不正确或已激活过，请查正后重新输入！');"
...
elseif rs_login("fwidmd5")<>fwid then
   ...
   response.Write "<script language=javascript>alert('输入正确，正在进入消费分红系统！');location.href='jypd50.asp'</script>"
   response.end
%>