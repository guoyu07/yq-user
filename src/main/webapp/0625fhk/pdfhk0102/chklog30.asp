...
sql="select * from jbk300 where pdid = '" & pdid & "'"
rs_login.open sql,conn0,1,1
if rs_login.eof and rs_login.bof then
   response.redirect "chklog50.asp?pdid="&pdid&"&pdpa="&pdpa&"&fwid="&fwid&""
   response.end
...
elseif rs_login("fwidmd5")<>fwid then
   ...
   response.Write "<script language=javascript>alert('输入正确，正在进入消费分红系统！');location.href='jypd30.asp'</script>"
   response.end
%>