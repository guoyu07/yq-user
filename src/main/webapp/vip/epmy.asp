...
elseif rs("sjb")+0<20 then
   response.write "<script language='javascript'>"
   response.write "alert('您好，您还没达到D套餐，暂时不能使用一币理财功能！\n\n您可以联系团队服务中心以95%的价格购买一币后进行操作！');"
   response.write "location.replace('vipup.asp?my="&username&"');"
   response.write "</script>" 
   response.end
elseif rs("cxt")<4 and rs("cxdate")>date() then
   response.write "<script language='javascript'>"
   response.write "alert('您好，您的诚信星为"&rs("cxt")&"，离取消[限制认购]时间还有"&rs("cxdate")-date()&"天，谢谢！');"
   response.write "history.go(-1);"
   response.write "</script>"
   response.end
elseif rs("username")=fbuser then
   response.write "<script language='javascript'>"
   response.write "alert('您好，不能认购自己的一币，请选择其它用户，谢谢！');"
   response.write "history.go(-1);"
   response.write "</script>" 
   response.end
...
<script language="JavaScript"> 
  ...
  if (hex_md5(Form.epnm3.value) !=="<%=md5(password3)%>") {      alert("二级密码不正确!");      return false;    }
  return true;  }  
 </script>
<p align="center">　</p>
...
<p align="center">　</p>
<p align="center"><font size="6">本次认购一币数量为：<font color="#FF0000"><%=txpay%></font></font><b><font color="#FF00FF" size="5" face="楷体">-将增加到您的“一币-余额”</font></b></p>
<p align="center"><b><font color="#800000" face="楷体" style="font-size: 20pt">实际只需要向发布方</font><font color="#000080" face="楷体" style="font-size: 20pt"><%=fbuser%></font><font color="#800000" face="楷体" style="font-size: 20pt">支付：</font><font color="#FF0000" face="楷体" style="font-size: 20pt"><%=txpay*0.9%></font><font face="楷体" style="font-size: 20pt">元</font></b></p>
<div align="center">
	<table border="0" width="80%" id="table1" cellspacing="1" bgcolor="#EFEFEF" height="135">
	<tr>
	<td><b><font color="#FF0000">重要提示：</font><font color="#0000FF">(发布方使用的收款方式是“</font><font color="#008000"><%=paybank%></font><font color="#0000FF">”)</font></b><p>
	本次下单须在您的用户名(认购方)：<b><font color="#008000"><%=username%> </font></b>的积分账户暂扣<b><font color="#FF0000"><%=txpay%></font></b>作为本次交易诚信金！<p>
	下单成功后必须在“<font color="#FF0000">2小时</font>”内向发布方<font color="#0000FF"><%=fbuser%></font>的指定银行账号打入不低于<b><font color="#FF0000"><%=txpay*0.9%></font></b>元！同时在登录平台点“一币-理财”下“认购明细”查看该笔交易状态-发布方<font color="#0000FF"><%=fbuser%></font>收款账号，并进行“我已付款<b><font color="#FF0000"><%=txpay*0.9%></font></b>-通知发布方确认”的操作！</td>
	</tr>
	</table>
...
<p align="center">　</p>
<p align="center"><b><a style="text-decoration: none" onClick="return confirm('提示：您确定取消本次交易吗？ ')" href="qxepjy.asp?ep=<%=jybh%>"><font color="#008000">返回交易市场</font></a></b></p>
<p align="center">　</p>
<script>
var secs = 60;  //6为倒计时秒数
for(i=0;i<=secs;i++) {
