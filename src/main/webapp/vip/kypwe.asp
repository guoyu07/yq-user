...
<%
if isNumeric(request("order"))=false then
   response.write "<script language='javascript'>"
   response.write "alert('订单号有误！');"
   response.write "history.go(-1);"
   response.write "</script>"
   response.end
elseif isNumeric(request("gwpay"))=false then
   response.write "<script language='javascript'>"
   response.write "alert('订单金额有误！');"
   response.write "history.go(-1);"
   response.write "</script>"
   response.end
elseif isNumeric(request("pid"))=false then
   response.write "<script language='javascript'>"
   response.write "alert('操作事项有误！');"
   response.write "history.go(-1);"
   response.write "</script>"
   response.end
end if
...
if request("pid")=1 then
   paylb="票务支付-"&request("order")
   title="来自科宇票务的订单"
else
   paylb="票务充值-"&request("order")
   title="来自科宇票务的充值"
end if
...
elseif rs1.eof and rs1.bof then
   ...
   response.write "</script>" 
   response.end
elseif rs1("pwdate")>date() then
   response.write "<script language='javascript'>"
   response.write "alert('您好，测试期间同一姓名及证件号30天内仅提供一次消费（不论金额大小），请于"&rs("pwdate")-date()&"天后再来，谢谢！');"
   response.write "history.go(-1);"
   response.write "</script>"
   response.end   
...
'StrSQL = "SELECT * FROM gcuser where name='"&rs1("name")&"' and userid='"&rs1("userid")&"' order by id asc"
'rsTest.Open StrSQL,Conn2,2,3
'While Not rsTest.EOF
'rsTest("pwdate")=date()+30
'rsTest.MoveNext
'Wend
'rsTest.UpdateBatch
...
'支付订单成功返回
if request("pid")=1 then
sn=md5(request("order")&"$#85546875#@$#%"&request("gwpay"))
Response.Write "<script language=""JavaScript"">alert('支付成功！');location.replace('http://www.kypwb.com/interface/YiBiPayReturn?act=payment&op=returnyibi&sn="&sn&"&paycode=success&payamount="&request("gwpay")&"&pid=1&order_sn="&request("order")&"&payuser="&rs1("username")&"');</script>"
Response.End
end if
...
sn=md5(request("order")&"$#85546875#@$#%"&request("gwpay"))
Response.Write "<script language=""JavaScript"">alert('充值成功！');location.replace('http://www.kypwb.com/interface/YiBiPayReturn?act=payment&op=returnyibi&sn="&sn&"&paycode=success&payamount="&request("gwpay")&"&pid=2&order_sn="&request("order")&"&payuser="&rs1("username")&"');</script>"
Response.End
end if
...
<form method="POST" action="kypwe.asp?ybf=actok" id="Form" name="Form" onSubmit="return checkdate1()">
	...
	<table border="0" id="table1" height="308" width="560">
		...
		<tr>
			...
			<td width="336" align="left" colspan="2"><b><font color="#0000FF"><%=gwuser%><input type="text" name="user" size="20"><input type="hidden" name="remark" size="5" value="11" readonly></font></b></td>
			</tr>
		<tr>
