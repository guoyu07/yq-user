...
rs.open sql,conn1,1,1
  if rs.eof and rs.bof then 
       response.write "<p align='center'> 还没有一币交易记录！</p>" 
   else 
...
%><div align="center">    
        ...
        <%do while not rs.eof%>
          ...
          <td valign="middle" align="center" width="450"><%if rs("zftime")<>"" then%>交易成功-卖出方<%=rs("payusername")%>已确认。<%elseif rs("ep")=1 then%><font color="#FF00FF">您已下单，等待您在</font><font color="#0000FF"><b><%=rs("rgdate")%></b></font><font color="#FF00FF">前付款，<b><a target="_blank" href="fbbank.asp?fb=<%=rs("payid")%>"><font color="#008000">点击查看卖出方收款账号！</font></a></b></font><%elseif rs("ep")=2 then%><%=rs("dfuser")%>--已付款，等待卖出方<%=rs("payusername")%>的确认中。。。<%end if%></td>
          <td valign="middle" align="center" width="250"><a onClick="return confirm('提示：您确定已向发布方 <%=rs("payusername")%> 打款 <%=rs("paynum9")%> 元了吗？')" href="myepok.asp?ep=<%=rs("payid")%>"><font color="#FF0000" size="2"><%if rs("ep")=1 then%>已打款<%=rs("paynum9")%>元，通知卖出方确认。<%end if%></font></a></td>
          </tr>
          <% i=i+1 
...
<p align="center">
<a href="javascript:history.back();" style="text-decoration: none"><font color="#FF6600" style="font-size: 12pt; font-weight: 700">返回上一页</font></a><font color="#FF6600" style="font-size: 12pt; font-weight: 700">---<a href="epjysc.asp"><font color="#0000FF">到一币-交易市场</font></a>---<a href="epmcjl.asp"><font color="#008000">到一币-卖出明细</font></a></font></p>
<p align="center"></p>
<p align="center"></p>
</body> 
</html>