 ...
 * 类说明  
 */
public class ManageResourceAction extends BaseManageAction<Object> {
    ...
    // 附加管理
    private static final String FIRST_FJGL_URL    = "FIRST_FJGL_URL";
    // 信息管理
	...
	public void initResources() {
       ...
       // Resource r = (Resource) ServletActionContext.getRequest().getSession().getAttribute(ROOT_RESOURCE);
        ...
        for (Resource res : list) {
            // 附加管理
            if (("FJGL").equals(res.getResoCode())) {
                for (Resource child : list) {
                    if (res.getResourceId()==child.getId()) {
                        if (request.getSession().getAttribute(FIRST_FJGL_URL) == null) {
                            request.getSession().setAttribute(FIRST_FJGL_URL, child.getResoUrl());
                            break;
                        }
                    }
                }
