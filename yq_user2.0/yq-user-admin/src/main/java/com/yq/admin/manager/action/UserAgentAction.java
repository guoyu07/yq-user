...
public class UserAgentAction extends BaseManageAction<Gcuser> {
	...
	public String useragent(){
		...
		if(adminService.userAgent(user,state)){
			if(state==1){
				AdminOperateLog log= new AdminOperateLog(super.getUserName(),super.getUserSession().getSessionId(), new Date(), AdminGlobal.USER_AGENT, "封号账户:"+user+",备注:"+URLDecoderUtil.decode(resionMassage,"utf-8"),user);
				adminService.addAdminOperateLog(log);
			}
			if(state==0){
				AdminOperateLog log= new AdminOperateLog(super.getUserName(),super.getUserSession().getSessionId(), new Date(), AdminGlobal.USER_AGENTCANCEL, "解封账户:"+user+",备注:"+URLDecoderUtil.decode(resionMassage,"utf-8"),user);
				adminService.addAdminOperateLog(log);
			}
		}
