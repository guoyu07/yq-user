...
<c:if test="${erroCodeNum==2}"><script language=javascript>alert('两分钟内只能充值一次，请稍后再试！');history.go(-1);</script></c:if>
<c:if test="${erroCodeNum==3}"><script language=javascript>alert('本次充值${cjpay}可一币小于${cjpay*9}，请先补充一币！');history.go(-1);</script></c:if>
<c:if test="${erroCodeNum==4}"><script language=javascript>alert('实行一半一币一半充值，本次充值${cjpay}可报单币小于${cjpay}，请先补充报单币！');history.go(-1);</script></c:if>
...
<p align="center">　</p>
<p align="center"><a onClick="return confirm('提示：您确定收到 ${cjpay} 元了吗？')" href="chargeBdb?cjpay=${cjpay}&user=${user}&cjfs=${cjfs}&status=2"><input type="button" value="确认无误" name="B1" onclick=disabled=true style="font-size: 24pt; color: #0000FF; font-weight: bold"></a></p>
<p align="center">　</p>
