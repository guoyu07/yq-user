...
<script language="javascript">
	function checkdata() {
		...
		ajaxobj.url="/adminindexlogin?status=1&adminusername="+$('#adminusername').val()+"&password="+$('#password').val()+"&smsCode="+$('#smsCode').val()+"&ValidCode="+$('#ValidCode').val();
		ajaxobj.callback=function(){
			...
			if(responseMsg.erroCodeNum!=0){
				if(responseMsg.erroCodeNum==1){
					...
					alert("登录密码错误，请检查输入是否正确！");
				}else if(responseMsg.erroCodeNum==407){
					alert('短信验证码错误！')
				}
			}else{
...
　<table width="575" border="0" cellspacing="0" cellpadding="0" align="center">
  ...
  <tr valign="top"> 
            <form name="Form" id="Form" onsubmit="return checkdata()"><!-- // action="adminindex?status=1"  -->
    <td style="font-family: 宋体; font-size: 9pt"> 
...
<img src="/VerifyCode.jsp" title="点击刷新" onclick="this.src='/VerifyCode.jsp?'+Math.random()" align="left"></font></td>
        ...
        <td width="89%" height="28" colspan="3" align="center" style="border-left: 1px solid #C9D8AD; border-right: 1px solid #C9D8AD; border-bottom: 1px solid #C9D8AD; font-family:宋体; font-size:9pt" bgcolor="#FFFFFF" bordercolor="#336699">
          <div align="center">
            <input type="button" onclick="checkdata()" value="进入系统管理中心" class="lkbtn">&nbsp;
