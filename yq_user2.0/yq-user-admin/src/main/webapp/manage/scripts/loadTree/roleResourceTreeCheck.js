...
function loadResourceTree(){
	...
	$.ajax({
		   type: "POST",
		   url: "/resource/loadResourceTree?paramId="+resourceId+"&node=",
		   success: function(msg){
			    zNodes = eval(msg);
			  	treeObj = $.fn.zTree.init($("#resourceTree"), setting, zNodes);
				treeObj.expandNode(treeObj.getNodes()[0], true, true, true);
				treeObj.setting.async.url = ctx+"/resource/loadResourceTree?paramId="+resourceId+"&node="+treeObj.getNodes()[0].id;
...
function getUrl(treeId, treeNode) {
	var resourceId=$("#resourceId").val();
	return "/resource/loadResourceTree?paramId="+resourceId+"&node="+treeNode.id;
}
