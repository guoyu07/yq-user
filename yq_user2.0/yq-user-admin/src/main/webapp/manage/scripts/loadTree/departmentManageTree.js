...
function loadDepartmentTree(){
	...
	$.ajax({
		   ...
		   success: function(msg){
				...
				treeObj.setting.async.url = "/department/loadTree.action?node="+treeObj.getNodes()[0].id;
...
function selectNodes(){
	...
	if (nodes.length>0) {
		...
		//默认首次加载
		$.ajax({
			  url: "/department/listPage.action?treeId="+treeObj.getSelectedNodes()[0].id+"&ajax=true",
			  cache: false,
...
function zTreeOnClick(e, treeId, treeNode) {
	  ...
	  $.ajax({
		  url: "/department/listPage.action?treeId="+treeNode.id+"&ajax=true",
		  cache: false,
		  success: function(html){
		    $("#biaoge").html(html);
		  }
