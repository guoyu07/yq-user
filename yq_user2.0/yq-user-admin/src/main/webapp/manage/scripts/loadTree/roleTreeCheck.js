...
function loadResourceTree(){
	...
	$.ajax({
		   type: "POST",
		   url: "/user/loadTree.action?paramId="+userId+"&node=",
		   success: function(msg){
			    zNodes = eval(msg);
			  	treeObj = $.fn.zTree.init($("#resourceTree"), setting, zNodes);
				treeObj.expandNode(treeObj.getNodes()[0], true, true, true);
				treeObj.setting.async.url = "/user/loadTree.action?paramId="+userId+"&node="+treeObj.getNodes()[0].id;
...
function getUrl(treeId, treeNode) {
	var userId=$("#userId").val();
	return "/user/loadTree.action?paramId="+userId+"&node="+treeNode.id;
}
