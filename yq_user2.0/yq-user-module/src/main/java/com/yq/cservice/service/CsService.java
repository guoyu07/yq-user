...
public class CsService {
	...
	private DatecjDao datecjDao;
  	...
  	private Cache<String,Session> csUserMap = CacheBuilder.newBuilder().expireAfterAccess(24, TimeUnit.HOURS).maximumSize(2000).build();

  	public Session getLoginCsUserName(String sessionId){
  		Session csSession = csUserMap.getIfPresent(sessionId);
		return csSession;
	}
	/**
	 * 管理员登录
	...
	public boolean csLogin(String userName,String password,String sessionId){
		...
		if(fcxt!=null){
			Session csSession = new Session(userName, System.currentTimeMillis(), sessionId);
			csUserMap.put(sessionId, csSession);
			return true;
		}else{
			return false;
		}
