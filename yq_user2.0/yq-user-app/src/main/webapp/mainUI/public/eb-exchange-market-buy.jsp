...
<c:if test="${erroCodeNum==2000}"><script language=javascript>alert('下单认购一币-"${txpay.paynum}"成功，请您及时付款，建立诚信交易渠道！');location.replace('epmyjl?status=1');</script></c:if>
    ...
    <div class="dialog-widget">
      ...
      <div class="dialog-wrap-border main-widget customer-service-center">
        <form class="widget-form" method="POST" action="epmy?ep=${ep}&status=1" name="Form" onSubmit="return checkdate()">
          ...
          <p class="button-line mt15 tc"><a class="widget-button-small" style="text-decoration: none" onclick="return confirm('提示：您确定取消本次交易吗？？ ')" href="epjysc?status=1">返回交易市场</a></p>
        </form>
      </div>
    </div>
  </div>
