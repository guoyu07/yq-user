...
<!-- <ul class="nav-secondary">
    ...
    <%--  <ul class="nav-secondary JQ_navSecondary">
     <%for (Resource re : secondResource) {
				%>
				 <li>
        			<a class="root" href="<%=re.getResoUrl()%>?secondThisState=<%=re.getId()%>"><%=re.getResoUrl()%></a>
      			</li>
				<li <% if(thisState.equals(re.getId()+"")){%> class="active" <%} %>><a href='<%=re.getResoUrl()%>?thisState=<%=re.getId()%>'><%=re.getResoName()%></a></li>
				<%
		}%>
    </ul> --%>
     <ul class="nav-secondary JQ_navSecondary">
